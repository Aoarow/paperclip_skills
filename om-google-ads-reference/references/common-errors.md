...
| **No error — `campaign.end_date_time` is simply missing from the response** | v23 omits it for campaigns without an end date. v22's `end_date` returned the sentinel `2037-12-30` instead. | Treat a missing value as "no end date" (`NULL`). Never compare against `2037-12-30` any more. Observed 2026-09-16 when `google_ads_sync.py` moved to v23; historical rows were set to `NULL`. |
...
| **No error — update reports success, read-back shows the old value** | The update mask omitted the field. Field masks generated from the object (e.g. `protobuf_helpers.field_mask`) drop fields set to their proto default: `False`, `0`, `""`. Setting `primary_for_goal = False` or `biddable = False` this way changes nothing. | Always set the update mask **explicitly** (`update_mask.paths.append("biddable")` / REST `updateMask`). Never trust the success response; read the value back. |
| `RESOURCE_ALREADY_EXISTS` re-linking a `BUSINESS_NAME`, and GAQL shows no link | The first link succeeded; Google hides business-name links from GAQL while reviewing them | Stop. Nothing to repair. Confirm "under review" in the UI. Never build a workaround that forces the status — observed 2026-09-13, it cost 19 agent runs. |
| `MutateError` *"Mutates are not allowed for the requested resource"* on a conversion action | System-managed action, e.g. the `GOOGLE_HOSTED` lead-form action | Change it in the Google Ads UI. Not a permission problem, not retryable. |
| **No error — a `GOOGLE_HOSTED` action (e.g. "Click-to-Call") shows `primary_for_goal = true`** | Google creates and manages these actions. With `include_in_conversions_metric = false` and a non-biddable goal (`customer_conversion_goal.biddable` unset), it counts nowhere. | **Not a second primary, not a drift, nothing to fix.** Check "exactly one counted primary" on `primary_for_goal AND include_in_conversions_metric` only. Do not report it or request a change. Reported twice on DOW (LEXA-847, LEXA-903). |
